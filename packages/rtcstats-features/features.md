...
AEC3 reports a fixed sentinel of about `0.176` dB when the estimator has no estimate, i.e. when no echo path was present. Those samples are excluded rather than averaged in, and the median is used instead of the mean because the series is a plateau with re-convergence ramps rather than noise around a constant, which makes a mean a function of the `getStats` polling rate. Note that this is the linear-filter ERLE measured before the spectral suppressor, so it understates total cancellation.
