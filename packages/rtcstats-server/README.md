...
(clientside monitoring), server-side dump collection
...
See [rtcstats-features](../rtcstats-features) for the feature extraction which previously
used to be integrated with the server.
...
* the S3 configuration for storing the files. If not configured the files will only be
...
This section describes the file format in **version 3**. See below for older file formats and upgrading
...
* Third element is the arguments to the call or event.
...
* Personally identifiable information SHOULD be obfuscated.
  * IP addresses in SDP, `onicecandidate`, `addIceCandidate`, or `RTCStatsReport`.
  * Device labels from `enumerateDevices` or `getUserMediaOnSuccess`.
  * Servers SHOULD perform obfuscation; clients MAY.
